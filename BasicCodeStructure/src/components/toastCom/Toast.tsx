...
import CONSTANTS from '@root/src/data/constants';
...
const Toast = (props: any) => {
  ...
  const setToastType = (messageTitle = CONSTANTS.SUCCESS, type = CONSTANTS.SUCCESS) => {
    let color = '';
    if (type === CONSTANTS.ERROR) { color = COLORS.red; }
    if (type === CONSTANTS.SUCCESS) { color = COLORS.green2; }
    setToastColor(color);
  ...
  return (
    <Animated.View style={[styles.tost, { transform: [{ translateY: animatedValue }], backgroundColor: toastColor }]}>
